...
while i < data_len:
    ...
    for j in range(0, len_uni_prices):
        ...
        while ss <= len_real_prices:
            price_buy_vol = price_buy_count = price_sell_vol = price_sell_count = price_delta = price_delta_percent = price_volume = price_trade_count = 0
...
df_test = pd.DataFrame(
        imb_candles,
        columns=['open time', 'close time', 'duration', 'open', 'high', 'low', 'close',
...
fig = go.Figure(data=go.Candlestick(x=np.unique(df_test['open time'].values),
        open=np.unique(df_test['open'].values),
        high=np.unique(df_test['high'].values),
        low=np.unique(df_test['low'].values),
        close=np.unique(df_test['close'].values)))
fig.update(layout_xaxis_rangeslider_visible=False)
fig
# -

np.unique(df_test['open'].values)
